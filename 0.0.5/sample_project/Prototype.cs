...
namespace GXPEngine
{
    class Prototype : Canvas
    {
        ...
        int AddCircle(ILuaState lua)
        {
            ...
            float bounciness = (float)Convert.ToDecimal(sbounciness);
            Bumbers circle = new Bumbers(radius, new Vec2(positionX, positionY));
            AddChild(circle);
            _bumberL.Add(circle);
            return 0;
        }
        ...
        void Update()
        {
            ...
            {
                _lines = null; _bumberL = null; _squaresL = null; /*squareList = null*/; _triangleL = null; //triangleList = null;
                this.Destroy();
            }
